...
fn compute_normals_corner() {
    ...
    let top = vec3f(0, y, textureLoad(terrain_heightmap_rb, coords_top_right + vec2i(0, -1), 0).r);
    ...
    textureStore(calculated_normals_lt, coords_top_left, vec4f(normal, 0.0));
    textureStore(calculated_normals_rt, coords_top_right, vec4f(normal, 0.0));
    textureStore(calculated_normals_lb, coords_bottom_left, vec4f(normal, 0.0));
    textureStore(calculated_normals_rb, coords_bottom_right, vec4f(normal, 0.0));
}
